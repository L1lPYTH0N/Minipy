...
const BotName = 'Lu BOT 1.0'; 
const instagram = 'http://www.instagram.com/lilpyth0n'; 
const aktif = '08:00 - 22:00';
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:1.0\n' 
            + 'FN:LilPython\n' // Mi nombre
            + 'ORG:Lu BOT 1.0;\n' // Nombre del bot
            + 'TEL;type=CELL;type=VOICE;waid=12603763088:+1 (260) 376-3088\n' // Numero de whatsapp-CONTACTO
            + 'END:VCARD'
prefix = '*','#','+'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `¡Hola1 @${num.split('@')[0]}\n Bienvenido a *${groupName}*\n\n Deberas presentarte con EDAD, NOMBRE, DE DONDE ERES`
				...
				teks = `Se salio una puta del grupo @${num.split('@')[0]} JAJAJAJAJA� 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: 'Espera un momento, no hagas spam pedazo de mierda',
					success: '️❬ ✔ ❭ Esta hecho 🖤',
					levelon: '❬ ✔ ❭ *leveling* *activado*',
					leveloff: ' ❬ X ❭  *leveling* *desactivado*',
					levelnoton: '❬ X ❭ *eso esta activado*',
					levelnol: '*error* 0 °-°',
					error: {
				stick: '*Vuelve a intentarlo ^_^*',
				Iv: 'No te entendi we :c'
				},
				only: {
					group: '[❗] Solamente puede ser usado en grupos ❌',
					premium: '[❗] Comando para gente premium we*',
					mod: '[❗] Este comando es solo para mods we*',
					benned: 'Has sido baneado, contacta con el administrador',
					ownerG: 'Solo Python puede usar ese comando',
					ownerB: 'Solo Python Puede usar ese comando',
					userB: `──「 LISTA 」──\nOlá Kak !\n Disculpa, no estas registrado. Registrate de la siguiente forma :, \n\nCommand : ${prefix}daftar (Nombre)|idade\nEjemplo : ${prefix}daftar ✞︎Python✞︎|17\n\n──「 BOT MAN 」──`,
			...
			const ownerNumber = ["525586805544@s.whatsapp.net","525586805544@s.whatsapp.net"] // Recoloque o seu numero
			const mod = [ownerNumber,"525586805544@s.whatsapp.net","525586805544@s.whatsapp.net"]// não sei o que eu isso 
			const adminbotnumber = ["525586805544@s.whatsapp.net","525586805544@s.whatsapp.net"]// admin bot numero
			const frendsowner = ["558981210976@s.whatsapp.net","558981246187@s.whatsapp.net"]// amigo do criador 
			const premium = ["525586805544@s.whatsapp.net","558981246187@s.whatsapp.net","558981204807@s.whatsapp.net","558981213553@s.whatsapp.net","554999498331@s.whatsapp.net","558981309535@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('¿En serio viejo?')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`tchau racista ${sender.split("@")[0]} Pedazo de mierda, ya te cargo la kk`)
		setTimeout( () => {
		...
		if (isGroupAdmins) return reply('*AVISANDO A LOS ADMINISTRADORES, ALERTA DE LINK, ATAQUE DE BINARIOS PROGRAMADO*')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} TE IRAS A LA MIERDA DE AQUI EN 5`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 DILE ADIOS A TU GRUPO HIJO DE PUTA 🤙")
		}, 0)
	}
		...
		if (!isGroup) return
		if (!isAntiLink) return
		if (isGroupAdmins) return reply('ESTA PROHIBIDO EL USO DE LINKS EN EL GRUPO *AVISANDO A ADMINISTRADORES* 🙂')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} TE VAS A IR A LA MIERDA DE AQUI`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo Adios asquerosidad 🤙")
		}, 0)
	}
	
	if (messagesC.includes("Como estan?")){
			client.updatePresence(from, Presence.composing)
			reply("Todo bien y tu?")
	}
	
		if (messagesC.includes("Que hacen?")){
			client.updatePresence(from, Presence.composing)
			reply("Observando tu hermosura☀️")
	}
			
		if (messagesC.includes("Yair")){
			client.updatePresence(from, Presence.composing)
			reply("Puto mierda naco prieto y estupido ")
	}
	
		if (messagesC.includes("Listo")){
			client.updatePresence(from, Presence.composing)
			reply("Bien hecho, crack")
	}
	
		if (messagesC.includes("Hola")){
			client.updatePresence(from, Presence.composing)
			reply("Hola campeon, eres el mejor ☀️")
	}
		...
		if (messagesC.includes("Mierda")){
			client.updatePresence(from, Presence.composing)
			reply("Cual es tu problema?:c")
	}
	
		if (messagesC.includes("busco novia")){
			client.updatePresence(from, Presence.composing)
			reply("Largate de aqui cagada, no es tinder ")
	}
		...
		if (messagesC.includes("*Python")){
			client.updatePresence(from, Presence.composing)
			reply(" Ese tipo es hermoso"")
	}
